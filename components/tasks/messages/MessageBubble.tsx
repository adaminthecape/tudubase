...
type MessageBubbleProps = Item<Task> & {
  variant: 'sent' | 'received';
};

export default function MessageBubble(props: MessageBubbleProps) {
  const { variant, id, name, notes, updatedAt, createdAt } = props;
  ...
  return (
    <Box sx={{ maxWidth: '60%', minWidth: 'auto' }}>
      ...
      >
        ...
        <Typography level="body-xs">{updatedAt}</Typography>
      </Stack>
      {name ? (
        <Sheet
          variant="outlined"
        ...
        >
          <Stack direction="row" spacing={1.5} sx={{ alignItems: 'center' }}>
            ...
            <div>
              <Typography sx={{ fontSize: 'sm' }}>{name}</Typography>
